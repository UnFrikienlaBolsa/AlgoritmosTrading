...
for cuentaArray in range (155,155+80):
    ...
    fechasCopiar = fechasCopiar[valorAjuste:]
    ...
    if (votacion > 2):
        ...
        if ((((max_test-cierreEvaluar)/cierreEvaluar))>0.035):
            ganas = ganas + 1
            rentabilidad = 0.035
            print ('INVERTIR\t'+indice+'\t'+str(cierreEvaluar)+'\t'+str(fechaEvaluar)+'\t'+str(rentabilidad))
        else:
            cierreReal = y_test[4]
            rentabilidad = (cierreReal-cierreEvaluar)/cierreEvaluar
            print ('INVERTIR\t'+indice+'\t'+str(cierreEvaluar)+'\t'+str(fechaEvaluar)+'\t'+str(rentabilidad))
        total = total + 1
...
#if (total>0):
#    print (indice+','+str(total)+','+str(dinero)+','+str(beneficio/dinero)+','+str(beneficio)+','+str(beneficio/total)+','+str(0)+','+str(ganas))
